...
try
    ...
    if isempty(job.fname)
        ...
        if job.opts.append
            % changed from nam1 = fullfile(pwd,[nam job.opts.ext]);
    ...
    %-Get print options
    %----------------------------------------------------------------------
    %opts = {nam1,'-noui','-painters', job.opts.opt{:}}; %changed by andrea
    ...
    opts = {nam1, '-noui', '-painters', '-djpeg'};
    %-Print
    %----------------------------------------------------------------------
    %if isdeployed
    %    deployprint(fg,opts{:});
        %deployprint(fg, '-djpeg', '-r100', nam);
    %else
        
        print(fg,  opts{:});
        %print(fg, '-djpeg', '-r100', nam)
    %end
