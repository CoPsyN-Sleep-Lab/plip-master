...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.83.04), Tue Jan 16 12:02:57 2018
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/brookestaveland/Dropbox/SNHC+PANLab/SNHC/psychopy/EmotionNonconscious-task/noncon.psyexp',
...
# Setup the Window
win = visual.Window(size=[1620, 1620], fullscr=False, screen=0, allowGUI=True, allowStencil=False,
...
# Initialize components for Routine "noncon"
nonconClock = core.Clock()
noncon_image = visual.ImageStim(win=win, name='noncon_image',
    image='sin', mask=None,
...
for thisTrial in trials:
    ...
    #------Prepare to start Routine "noncon"-------
    t = 0
    nonconClock.reset()  # clock 
    frameN = -1
    # update component parameters for each repeat
    noncon_image.setImage(noncon)
    # keep track of which components have finished
    nonconComponents = []
    nonconComponents.append(noncon_image)
    for thisComponent in nonconComponents:
        if hasattr(thisComponent, 'status'):
            thisComponent.status = NOT_STARTED
    
    #-------Start Routine "noncon"-------
    continueRoutine = True
    while continueRoutine:
        # get current time
        t = nonconClock.getTime()
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        # *noncon_image* updates
        if frameN >= 0 and noncon_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            noncon_image.tStart = t  # underestimates by a little under one frame
            noncon_image.frameNStart = frameN  # exact frame index
            noncon_image.setAutoDraw(True)
        if noncon_image.status == STARTED and frameN >= (noncon_image.frameNStart + 1):
            noncon_image.setAutoDraw(False)
    ...
    #-------Ending Routine "noncon"-------
    for thisComponent in nonconComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "noncon" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    while continueRoutine:
        ...
        # *image* updates
        if (noncon_image.status==FINISHED) and image.status == NOT_STARTED:
            # keep track of start time/frame for later
            image.tStart = t  # underestimates by a little under one frame
            image.frameNStart = frameN  # exact frame index
            image.setAutoDraw(True)
        if image.status == STARTED and t >= (image.tStart + 0.150):
    ...
    for thisComponent in neutralComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "neutral" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
